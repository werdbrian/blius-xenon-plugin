...
static bool       g_postBashThrowStored = false;
static float      g_postBashThrowRange  = 20.f;
static float      g_postBashHoldEnd     = 0.f;
...
static float      g_comboWhipDeadline = 0.f;
...
static float    g_repairHpPctTank = 70.f;   // heal tank below this % HP
static float    g_repairHpPctDps  = 50.f;   // heal DPS/support below this % HP
static float    g_tankHpThreshold = 400.f;  // max HP above this = tank
...
static bool     g_dbgWhipHpOk      = false; // target HP <= whipKillHp
...
extern "C" void on_load()
{
    ...
    g_repairHpPctTank  = Config::GetFloat("repairHpPctTank", 70.f);
    g_repairHpPctDps   = Config::GetFloat("repairHpPctDps",  50.f);
    g_tankHpThreshold  = Config::GetFloat("tankHpThreshold", 400.f);
    g_repairRange      = Config::GetFloat("repairRange",      25.f);
    g_postBashThrowRange = Config::GetFloat("postBashThrowRange", 20.f);
...
extern "C" void on_unload()
{
    ...
    Config::SetFloat("repairHpPctTank",   g_repairHpPctTank);
    Config::SetFloat("repairHpPctDps",    g_repairHpPctDps);
    Config::SetFloat("tankHpThreshold",   g_tankHpThreshold);
    Config::SetFloat("repairRange",        g_repairRange);
    Config::SetFloat("postBashThrowRange", g_postBashThrowRange);
...
extern "C" void on_frame(float dt)
{
    ...
    if (g_repairHoldEnd > 0.f && (g_combo != CB_IDLE || g_bashPhase != BA_IDLE))
    {
    ...
    if (held && g_autoRepair && g_repairTarget >= 0 && g_repairHoldEnd <= 0.f && now >= g_repairCdEnd
        && g_combo == CB_IDLE && g_bashPhase == BA_IDLE)
    {
        if (local.IsValid())
        {
            g_dbgS1OnCd = local.GetSkill2Cooldown().IsOnCooldown();  // whip lives in slot 2
    ...
    if (g_autoCombo && g_combo == CB_IDLE && now >= g_comboCdEnd
        && keyDown && g_targetValid && g_targetVisible && g_targetDist <= g_comboRange
        && g_targetHp > 0.f && g_targetHp <= g_comboKillHp
        && nearbyEnemyCount < g_bashGroupMin && g_bashPhase == BA_IDLE)
    {
        g_combo  = CB_FLAIL1;
        g_comboT = 0.f;
    }
    ...
    if (g_combo != CB_IDLE)
    {
        ...
        {
            case CB_FLAIL1:
                if (g_targetVisible) PressGameButton(GameButton::LMouse);
            ...
            case CB_FLAIL2:
                ...
                if (g_comboT >= kCbFlail2) { g_combo = CB_WHIP; g_comboT = 0.f; g_comboWhipDeadline = now + 0.5f; }
                break;
            case CB_WHIP:
                ReleaseGameButton(GameButton::LMouse);
                ReleaseGameButton(GameButton::RMouse);
                ...
                ReleaseGameButton(GameButton::Crouch);
                {
                    SkillCooldown s1 = local.IsValid() ? local.GetSkill2Cooldown() : SkillCooldown{};  // whip = slot 2
                    bool canFire = g_targetVisible && !s1.IsOnCooldown();
                    if (canFire)
                        PressGameButton(GameButton::Skill1);
                    else
                    {
                        ReleaseGameButton(GameButton::Skill1);
                        g_comboT = 0.f;  // freeze hold timer until whip can fire
                    }
                }
                if (g_comboT >= kCbWhip || now >= g_comboWhipDeadline)
                {
                    ReleaseGameButton(GameButton::Skill1);
                    g_combo      = CB_IDLE;
                    g_comboT     = 0.f;
    ...
    bool engaged      = held || duelClose || blockPending;
    ...
    // -------------------------------------------------------
    if (!keyDown && g_bashPhase != BA_IDLE)
    {
        ReleaseGameButton(GameButton::RMouse);
        ReleaseGameButton(GameButton::LMouse);
        g_bashPhase  = BA_IDLE;
    ...
    bool whipOffCd     = !(local.IsValid() && local.GetSkill2Cooldown().IsOnCooldown());  // whip = slot 2
    bool whipInRange   = g_targetVisible && g_targetDist >= g_whipMinDist && g_targetDist <= g_whipMaxDist;
    bool whipHpOk      = g_targetHp > 0.f && g_targetHp <= g_whipKillHp;
    bool whipClear     = g_targetBarrier <= 0.f && !g_targetShielded;
    bool whipWantsRaw  = g_autoWhip && g_targetValid && whipOffCd && whipInRange && whipHpOk && whipClear;

    g_dbgWhipReady   = whipOffCd;
    g_dbgWhipInRange = whipInRange;
    g_dbgWhipHpOk    = whipHpOk;
    ...
    bool blockActive = blockPending && g_combo == CB_IDLE && !bashActive;
    if (bashActive || blockActive)
        PressGameButton(GameButton::RMouse);
    else if (g_combo == CB_IDLE)
        ReleaseGameButton(GameButton::RMouse);
    ...
    bool comboWaiting = g_autoCombo && g_combo == CB_IDLE && now >= g_comboCdEnd
        && keyDown && g_targetValid && g_targetDist <= g_comboRange
        && g_targetHp > 0.f && g_targetHp <= g_comboKillHp;
    ...
    bool whipConditions = !bashActive && !comboWaiting && !preMeleeActive && meleeFirstOk
    ...
    // -------------------------------------------------------
    // Post-bash shield throw: LMouse at extended range, consumes stored throw
    if (!bashActive && g_combo == CB_IDLE && keyDown && g_targetValid && !blockActive)
    {
        if (g_postBashThrowStored && g_targetDist <= g_postBashThrowRange)
        {
            g_postBashHoldEnd     = now + kBashHold;
            g_postBashThrowStored = false;
        }
        if (g_postBashHoldEnd > 0.f)
        {
            AimAtBone(g_cachedTarget, g_bestBone, g_meleeStiffness);
            if (now < g_postBashHoldEnd)
                PressGameButton(GameButton::LMouse);
            else
            {
                ReleaseGameButton(GameButton::LMouse);
    ...
    else if (g_postBashHoldEnd > 0.f) { /* post-bash throw owns LMouse */ }
...
extern "C" void on_render()
{
    ...
    if (g_showDebug)
    {
        ...
        bool   s1cd  = lpD.IsValid() && lpD.GetSkill2Cooldown().IsOnCooldown();  // whip = slot 2
        ...
        bool dWhipReady   = !s1cd;
        bool dWhipInRange = g_targetVisible && g_targetDist >= g_whipMinDist && g_targetDist <= g_whipMaxDist;
        bool dWhipHpOk    = g_targetHp > 0.f && g_targetHp <= g_whipKillHp;
        bool dWhipClear   = g_targetBarrier <= 0.f && !g_targetShielded;
        bool dWhipWant    = g_autoWhip && g_targetValid && dWhipReady && dWhipInRange && dWhipHpOk && dWhipClear;
        ...
        // Skill-slot diagnostic — fire your whip and watch which slot's cooldown ticks.
        // That tells us which GetSkillNCooldown() the whip really lives in. cur/max en=ticking
        by += 18.f;
        SkillCooldown sc1 = lpD.IsValid() ? lpD.GetSkill1Cooldown() : SkillCooldown{};
        SkillCooldown sc2 = lpD.IsValid() ? lpD.GetSkill2Cooldown() : SkillCooldown{};
        SkillCooldown sc3 = lpD.IsValid() ? lpD.GetSkill3Cooldown() : SkillCooldown{};
        TextBuilder<192> sk;
        sk.put("S1 ").putFloat(sc1.current,1).put("/").putFloat(sc1.max,1).put(" en:").put(sc1.enabled?"Y":"n")
          .put("  S2 ").putFloat(sc2.current,1).put("/").putFloat(sc2.max,1).put(" en:").put(sc2.enabled?"Y":"n")
          .put("  S3 ").putFloat(sc3.current,1).put("/").putFloat(sc3.max,1).put(" en:").put(sc3.enabled?"Y":"n");
        Draw::TextShadow(bx, by, Color::Cyan(), sk.c_str());
...

    {
        ...
        bool reactionActive = g_reactionTimer > 0.f;
        {
            ...
            if (useIdx >= 0 && tgt.IsValid())
            {
                ...
                // If target is visible but geometry raycast is blocked, a shield is in the way
                g_targetShielded = false;
                {
                    Vector3 camPos;
                    if (GetCameraPosition(camPos) && g_targetBodyPos.IsValid())
                        g_targetShielded = !IsPointVisible(camPos, g_targetBodyPos);
                }
...
extern "C" void on_menu()
{
    if (ImGui::CollapsingHeader("Brigitte"))
    {
        ...
        ImGui::SliderFloat("Tank HP Threshold",&g_tankHpThreshold, 100.f, 800.f);
