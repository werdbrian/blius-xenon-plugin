...
extern "C"
{
    ...
    __attribute__((import_module("core"), import_name("pulse_game_button")))
    void xn_pulse_game_button(int32_t bit, int32_t holdMs);
...
namespace xenon
{
    ...
    // RENDER-ONLY — call from on_render().
    // The view matrix is only refreshed during the render phase. Calling from
    // on_frame() returns false and writes zeros (silent until v1.x — see
    // changelog). For frame-phase logic that needs camera state, cache values
    // in on_render() and read the cache in on_frame().
    ...
    // RENDER-ONLY — call from on_render(). Returns screen coordinates even if
    // off-screen; use for bounding-box corners. See WorldToScreen note.
    ...
    // RENDER-ONLY — call from on_render(). Camera position extracted from the
    // view-projection matrix, which is only refreshed during the render phase.
    ...
    // RENDER-ONLY — call from on_render(). Normalized forward vector derived
    ...
    // RENDER-ONLY — call from on_render(). Raw 4x4 view-projection matrix
    // (row-major, 16 floats). Outside the render phase the matrix is not
    // refreshed; the binding returns false without writing.
    ...
    // Press a game button and schedule an automatic release after holdMs
    // milliseconds. Use this for abilities and other edge-triggered inputs
    // (Skill1 / Skill2 / Ult / Reload / Interact / Melee) — PressGameButton
    // on its own keeps the bit held, and the game only fires these on the
    // rising edge, so a held bit blocks every follow-up cast. PulseGameButton
    // handles the release automatically so plugins don't need a per-bit timer.
    inline void PulseGameButton(uint32_t bit, uint32_t holdMs = 50)
    {
        xn_pulse_game_button(static_cast<int32_t>(bit),
                             static_cast<int32_t>(holdMs));
    }
