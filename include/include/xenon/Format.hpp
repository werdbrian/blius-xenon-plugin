...
inline int float_to_str(float value, char* buf, int buf_size, int decimals = 1)
{
    ...
    // WASM without -mnontrapping-fptoint traps on i32.trunc_f32_u for NaN, +Inf,
    // or values >= 2^32. A trap inside on_render kills the plugin after 10 frames.
    // NaN comparisons are always false, so this single check covers all three.
    if (!(value < 4294967040.f))
    {
        if (pos + 3 < buf_size)
        {
            buf[pos++] = 'n';
